...
$HOST_USER_NAME@$VIRTUAL_SERVER_INSTANCE env BUILDDIR=$BUILDDIR USERID=$USERID TOKEN=$TOKEN REPO=$REPO APPNAME=$APPNAME COSENDPOINT=$COSENDPOINT COSBUCKETNAME=$COSBUCKETNAME OBJECTNAME=$OBJECTNAME WORKDIR=$WORKDIR HOST_USER_NAME=$HOST_USER_NAME 'bash -s ${BUILDDIR}/deploy.sh'
